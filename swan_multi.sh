...
if [[ $seqCenter = 'Broad' ]]; then
  data_dir="/project/lswanglab/adsp/data/broad-recalled"
...
if [[ $seqCenter = 'test' ]]; then
    ...
    bam_path1="$sample_path/${bam_file}.liba.chr${chr}.bam"
    bam_path2="$sample_path/${bam_file}.libb.chr${chr}.bam"
    bam_path3="$sample_path/${bam_file}.libc.chr${chr}.bam"
fi
...
then
  ...
  then
    ...
    if [[ $seqCenter = 'test' ]]; then
      ...
      $scripts_dir/swan_stat.R -c $chr -o $sample_path/$file_designation.chr$chr $bam_path1,$bam_path2,$bam_path3
  fi
fi
...
then
  if [[ $seqCenter = 'test' ]]; then
      ...
      $scripts_dir/swan_scan.R -r 0.5 -w 100 -c $chr -n $gap_file $ref_file $bam_path1,$bam_path2,$bam_path3
  fi
fi
...
then
  ...
  if [[ $seqCenter = 'test' ]]; then
    ...
    $scripts_dir/sclip_scan.R -a -q -c $chr -t $sample_path/$file_designation.chr$chr.stat -n $gap_file -o $sample_path/sclip_events/$sampleID $ref_file $bam_path1,$bam_path2,$bam_path3
  fi
fi
...
then
  if [[ $seqCenter = 'test' ]]; then
    ...
    $scripts_dir/swan_join.R -a -q -c a -t $sample_path/$file_designation.chr$chr.stat -m $sample_path/$bam_file.disc.txt -i $sample_path/$bam_file -j $sample_path/$bam_file.bigd.txt -l $sample_path/sclip_events/${sampleID}.sclip.RData -u $swanopt -v $bigdopt $ref_file $bam_path1,$bam_path2,$bam_path3
  fi
fi
